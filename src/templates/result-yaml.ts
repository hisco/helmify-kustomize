 ...
 * @param {string} packageId - The package ID for helpers
 * @param {ParametrizeConfigmap[]} parametrizeConfigmaps - Config maps to parametrize
 * @param {boolean} includeKustomizeFiles - Whether to include kustomize files
 * @param {boolean} hasTemplatedValuesYaml - Whether there's a templated values.yaml
 * @param {string} version - The version of helmify-kustomize
 * @returns {string} The content of the result.yaml file
 */
export const yamlResult = (chartPrefix: string , packageId: string , parametrizeConfigmaps: ParametrizeConfigmap[], includeKustomizeFiles: boolean , hasTemplatedValuesYaml: boolean, version?: string): string => {
...
{{- $values := merge .Values $templateValues }}

{{- /* Ensure user-provided globals take precedence */ -}}
{{- if .Values.globals }}
  {{- $_ := set $values "globals" .Values.globals }}
{{- end }}` 
: `{{- $values := .Values }}`}
...
{{- include "${packageId}.podLabels" (dict "manifest" $manifest "globals" $globals "Values" $values)}}
{{- include "${packageId}.podAnnotations" (dict "manifest" $manifest "globals" $globals "Values" $values)}}
