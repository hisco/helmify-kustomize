...
import { analyzeYamlAnchors, AnchorInfo, AnchorReference } from './anchor-ref';
import { generateValuesYamlTemplate, generateYamlFromatInfraHelpers } from './templates/helm-yaml-anchors';
...
export async function wrapKustomizeIntoHelm({
  ...
  const resultYamlContent = yamlResult(chartPrefixHash, thisPackageId, parametrizeConfigmaps, includeKustomizeFiles);
  // Write the result.yaml file to the templates folder
  ...
  // Track anchor references for later processing
  const anchorReferences: AnchorInfo[] = [];
    ...
    try {
      ...
      if (enabledDynamicAnchorReplacement) {
        const yamlTemplateFile = [
          generateYamlFromatInfraHelpers(thisPackageId, doc.toString()),
          generateValuesYamlTemplate(thisPackageId, doc.toString()),
        ].join(`\n`)
